...
//final Auth=FirebaseAuth.instance;
  ...
  void initState() {
    super.initState();
    classes=new List();
    _onClassesAddedSubscription=todoReference.onChildAdded.listen(_onClassesAdded);
    _onnClassesChangedSubscription=todoReference.onChildChanged.listen(_onClassesUpdated);
  }
...
//now i design the ui to the user
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        ...
        appBar: AppBar(
          ...
          title:Text('Your Classes',style: TextStyle(color: Colors.white,fontSize: 18.0),
          ),
          centerTitle: true,
        ),
        body: Center(
          child: ListView.builder(
            ...
            itemBuilder: (context,posi){
              return Card(
                margin: EdgeInsets.only(top:9.0,bottom: 9.0,left: 14.0,right: 14.0),
                      ...
                      children: <Widget>[
                        Expanded(
                            child: ListTile(
                              ...
                              leading: Column(
                                ...
                                children: <Widget>[
                                  CircleAvatar(
                                    backgroundColor: Colors.indigo,
                                    radius: 14.0,
                                    child: Text('${posi + 1}',style: TextStyle(color: Colors.black,
                                      //  backgroundColor: Colors.purple[50],
                                      fontSize:14.0,
                                    ),
                                    ),
                              ...
                              onTap:  ()=>_navigateClassInformation(context,classes[posi]),

                            ),
                        ),
                        IconButton(
                          icon: Icon(Icons.delete,color: Colors.red,),
                          onPressed: ()=>_deleteClass(context,classes[posi],posi),
                        ),
                        IconButton(
                          icon: Icon(Icons.edit,color: Colors.indigo,),
                          onPressed:   ()=>_navigateClass(context,classes[posi]),
                        ),
                      ],
                    ),
  ...
  void _onClassesUpdated(Event event){
    var oldClassValue=classes.singleWhere((todo)=>todo.key==event.snapshot.key);
    setState(() {
      classes[classes.indexOf(oldClassValue)]= new Todo.fromSnapshot(event.snapshot);
    });
  }
